...
void initialize_rand(TIN* ptr, size_t capacity, cutlass::Distribution::Kind dist_kind, uint32_t seed)
{
    ...
    if (dist_kind == cutlass::Distribution::Uniform)
    {
        ...
        cutlass::reference::device::BlockFillRandomUniform(ptr, capacity, seed, scope_max, scope_min, 0);
    }
    else if (dist_kind == cutlass::Distribution::Gaussian)
    {

        cutlass::reference::device::BlockFillRandomGaussian(ptr, capacity, seed, TIN(), TIN(1.0f));
    }
    else if (dist_kind == cutlass::Distribution::Sequential)
    {

        // Fill with increasing elements
        cutlass::reference::device::BlockFillSequential(ptr, capacity, TIN(1), TIN());
    }
}
...
void initialize_const(Element* ptr, size_t capacity, const Element& value)
{
    ...
    // Fill with all 1s
    cutlass::reference::device::BlockFillSequential(ptr, capacity, Element(), value);
}
            ...
            (abs_diff > abs_tol && relative_diff > rel_tol))
        {
            if (printDiffs)
                printf("[%d/%d] diff = %f, rel_diff = %f, {computed=%f, ref=%f}.\n",
                       ...
                       abs_diff,
                       relative_diff,
                       (float)(vector_Input[i]),
                       (float)(vector_Input_Ref[i]));
            errCount++;
            // return false;
        }
    }
