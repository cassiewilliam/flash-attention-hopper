...
fmhaForward(PrecType const *Q,
            CUTE_GRID_CONSTANT TiledCopyQ const tmaLoadQ,
            TileShapeQ tileShapeQ, 
            GmemLayoutQ gmemLayoutQ,
            SmemLayoutQ smemLayoutQ, 
            PrecType const *K,
            CUTE_GRID_CONSTANT TiledCopyK const tmaLoadK, 
            TileShapeK tileShapeK,
            GmemLayoutK gmemLayoutK, 
            SmemLayoutK smemLayoutK, 
            PrecType *S,
            TileShapeS tileShapeS, 
            GmemLayoutS gmemLayoutS,
            SmemLayoutS smemLayoutS, 
            int nTilesOfK,
            PrecType *V,
            CUTE_GRID_CONSTANT TiledCopyV const tmaLoadV,
            TileShapeV tileShapeV,
            GmemLayoutV gmemLayoutV,
            SmemLayoutV smemLayoutV,
            SmemLayoutVt smemLayoutVt,
            PrecType *O,
            CUTE_GRID_CONSTANT TiledCopyO const tmaStoreO,
            TileShapeO tileShapeO,
            GmemLayoutO gmemLayoutO,
            AccumType *mi_ptr,
            AccumType *sPrimePtr,
            ...
            float scale) 
{
  ...
  // will also read from SMEM. By default, this flag is disabled.
    ...
    // Required for verification ONLY.
        ...
        if (blockIdxY == 0) 
        {
            // Compute Online Softmax and NO Output Rescaling.
            onlineSoftmaxAndRescale<true, AccumType>(rowMax, rowSum, tSrS, tOrO, scale);
        }
        else
        { 
            // Compute Online Softmax and Output Rescaling.
        ...
        // issuing.
        cfk::gemm_ldbar(tiledMma1, convert_type<PrecType, AccumType>(tOrP), tOrV, tOrO, tma_load_mbar[1], phase);
    #endif
...
void fmhaForwardDevice(int SEQLEN,
                       int KEYLEN,
                       int NUMHEADS,
                       int BATCH,
                       ...
                       cudaStream_t stream = 0)
{
    ...
    auto B = int(BATCH);     // batch_size
    auto H = int(NUMHEADS);  // num_heads
    auto M = int(SEQLEN);    // sequence_len
    auto N = int(KEYLEN);    // 
    auto K = int(HEADDIM);   // head_dim
  ...
  // Disabled by default.
    ...
    using TiledMma0 = decltype(cute::make_tiled_mma(cute::GMMA::ss_op_selector<MmaA, MmaB, MmaC, Shape<bM, bN, bK>>(), MmaTileShape{}));

  #ifdef SINSMEM
    // USE SS version of GMMA for GEMM-II.
    using TiledMma1 = decltype(cute::make_tiled_mma(cute::GMMA::ss_op_selector<MmaA, MmaB, MmaC, Shape<bM, bK, bN>, GMMA::Major::K, GMMA::Major::MN>(), MmaTileShape{}));
  #else
    // USE RS version of GMMA for GEMM-II (Default).
    using TiledMma1 = decltype(cute::make_tiled_mma(cute::GMMA::rs_op_selector<MmaA, MmaB, MmaC, Shape<bM, bK, bN>, GMMA::Major::K, GMMA::Major::MN>(), MmaTileShape{}));
  #endif
    ...
    void const *kernel = (void const *)fmhaForward<PrecType,
                                                   AccumType,
                                                   TiledMma0,
                                                   TiledMma1,
                                                   decltype(tmaQ),
                                                   decltype(tileShapeQ),
                                                   decltype(gmemLayoutQ),
                                                   decltype(smemLayoutQ),
                                                   decltype(tmak),
                                                   decltype(tileShapeK),
                                                   decltype(gmemLayoutK),
                                                   decltype(smemLayoutK),
                                                   decltype(tileShapeS),
                                                   decltype(gmemLayoutS),
                                                   decltype(smemLayoutS),
                                                   decltype(tmaV),
                                                   decltype(tileShapeV),
                                                   decltype(gmemLayoutV),
                                                   decltype(smemLayoutV),
                                                   decltype(smemLayoutVt),
                                                   decltype(tmaO),
                                                   decltype(tileShapeO),
                                                   decltype(gmemLayoutO),
                                                   decltype(gmemLayoutMi),
                                                   ClusterShape>;

    // Compute and set dynamic shared memory size.
    auto smem_size = int(sizeof(SharedStorage<MmaA, decltype(smemLayoutQ), decltype(smemLayoutK), decltype(smemLayoutS), decltype(smemLayoutV)>));
...
template <typename PrecType, typename AccumType, int HEADDIM>
void fmhaForwardDeviceLoop(int SEQLEN,
                           int KEYLEN,
                           int NUMHEADS,
                           int BATCHSIZE,
                           PrecType const *Q,
                           ...
                           float scale)
{
    ...
    if (nStreams == 1)
    {
        fmhaForwardDevice<PrecType, AccumType, HEADDIM>(SEQLEN,
                                                        KEYLEN,
                                                        NUMHEADS,
                                                        BATCHSIZE,
                                                        Q,
                     ...
                     int nStreams)
{
    ...
    std::cout << "L = " << lLong << " : " << numHeads << " * " << batchSize << std::endl;
    ...
    double fmha_flops = double(4 * batchSize * numHeads * mLong * nLong * kLong) / double(1.0e9);

    // Run few times (warmup).
    devS = hostS;
    devD = hostD;
    devV = hostV;
    fmhaForwardDeviceLoop<PrecType, AccumType, HEADDIM>(m,
                                                        n,
                                                        numHeads,
                                                        batchSize,
                                                        devQ.data().get(),
    ...
    CUTE_CHECK_LAST();
    printf("CUTE_FMHA:     [%6.1f]Gflop/s  (%6.4f)ms\n", fmha_flops / cute_time, cute_time * 1000);
    ...
    if (refCheck)
    {
        ...
        devD = hostD;
        devS = hostS;
        testBed.compute(devQ.data().get(),
                        devK.data().get(),
                        devV.data().get(),
                        devS.data().get(),
                        devD.data().get(),
                        miRefHostOut.data(),
                        sPrimeRefHostOut.data(),
                        usePow2,
                        usePreScaling);
...
#ifdef COPYOUTMM0
      // Our intermediate write to S is unscaled. So scale it before checking with CUTLASS.
        if (usePreScaling)
        {
...
#ifdef COPYOUTMI
        // Our intermediate write to MI is scaled with log2e. So un-scale it before checking with CUTLASS.
        if (!usePow2)
        {
        ...
        // Our intermediate write to sPrime is not reciprocal. So invert it before checking with CUTLASS.
        for (int j = 0; j < sPrimeHostOut.size(); ++j)
        {
            sPrimeHostOut[j] = (1.0 / sPrimeHostOut[j]);
        }

        bool sumCheck = cfk::verify_tensor(sPrimeHostOut, sPrimeRefHostOut, printValues);
...
int main(int argc, char const **argv)
{
    ...
    {
        testFmhaForward<cutlass::half_t, 128>(seqLength,
                                              seqLength,
                                              numHeads,
                                              batchSize,
                                              iterations,
