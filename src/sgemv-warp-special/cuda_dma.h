...
#define SEQUENTIAL_EXECUTE(DO_SYNC, BYTES_PER_ELMT, DMA_THREADS)                                                                                                                                                                                            \
    ...
    {                                                                                                                                                                                                                                                       \
        ...
        {                                                                                                                                                                                                                                                   \
            CUDADMA_BASE::template do_xfer<DO_SYNC, ALIGNMENT>(src_temp, dst_temp, (this_thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) ? (this_thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) : MAX_BYTES_OUTSTANDING_PER_THREAD);                     \
        }                                                                                                                                                                                                                                                   \
        else                                                                                                                                                                                                                                                \
        {                                                                                                                                                                                                                                                   \
            CUDADMA_BASE::template do_xfer<false, ALIGNMENT>(src_temp, dst_temp, (this_thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) ? (this_thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) : MAX_BYTES_OUTSTANDING_PER_THREAD);                       \
        }                                                                                                                                                                                                                                                   \
    }                                                                                                                                                                                                                                                       \
    else                                                                                                                                                                                                                                                    \
    {                                                                                                                                                                                                                                                       \
        CUDADMA_BASE::template do_xfer<false, ALIGNMENT>(src_temp, dst_temp, (this_thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) ? (this_thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) : (this_thread_bytes ? MAX_BYTES_OUTSTANDING_PER_THREAD : 0)); \
    }                                                                                                                                                                                                                                                       \
    if (DO_SYNC)                                                                                                                                                                                                                                            \
...
#    ifdef CUDADMA_DEBUG_ON
            if ((CUDADMA_BASE::dma_tid == 1) && (CUDADMA_BASE::barrierID_full == 4) && (CUDADMA_BASE::is_dma_thread))
            {
                ...
                printf("bytes = %d\n",
                       (this_thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) ? (this_thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) : (this_thread_bytes ? MAX_BYTES_OUTSTANDING_PER_THREAD : 0));
            }
#    endif
            CUDADMA_BASE::template do_xfer<false, ALIGNMENT>(src_temp, dst_temp, (this_thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) ? (this_thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) : (this_thread_bytes ? MAX_BYTES_OUTSTANDING_PER_THREAD : 0));
        }
...
protected:
    ...
    __device__ __forceinline__ void copy_elmt(const char* RESTRICT src_ptr, char* RESTRICT dst_ptr, int dma_col_iters) const
    {
        ...
        CUDADMA_BASE::template do_xfer<false, ALIGNMENT>(src_ptr, dst_ptr, (thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) ? (thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) : (thread_bytes ? MAX_BYTES_OUTSTANDING_PER_THREAD : 0));
    }
...
protected:
    ...
    {
        int index_offset1 = elmt_byte_size * index_ptr[offset_index + (LAST_XFER ? (GATHER ? dma1_src_offs : dma1_dst_offs) : (GATHER ? dma1_src_iter_offs : dma1_dst_iter_offs))];
        int index_offset2 = elmt_byte_size * index_ptr[offset_index + (LAST_XFER ? (GATHER ? dma2_src_offs : dma2_dst_offs) : (GATHER ? dma2_src_iter_offs : dma2_dst_iter_offs))];
        int index_offset3 = elmt_byte_size * index_ptr[offset_index + (LAST_XFER ? (GATHER ? dma3_src_offs : dma3_dst_offs) : (GATHER ? dma3_src_iter_offs : dma3_dst_iter_offs))];
        int index_offset4 = elmt_byte_size * index_ptr[offset_index + (LAST_XFER ? (GATHER ? dma4_src_offs : dma4_dst_offs) : (GATHER ? dma4_src_iter_offs : dma4_dst_iter_offs))];

        TYPE1 tmp1 = *(const TYPE1*)((const char*)src_ptr + (LAST_XFER ? (GATHER ? index_offset1 : dma1_src_offs) : (GATHER ? index_offset1 : dma1_src_iter_offs)));
        TYPE1 tmp2 = *(const TYPE1*)((const char*)src_ptr + (LAST_XFER ? (GATHER ? index_offset2 : dma2_src_offs) : (GATHER ? index_offset2 : dma2_src_iter_offs)));
        TYPE1 tmp3 = *(const TYPE1*)((const char*)src_ptr + (LAST_XFER ? (GATHER ? index_offset3 : dma3_src_offs) : (GATHER ? index_offset3 : dma3_src_iter_offs)));
        TYPE2 tmp4 = *(const TYPE2*)((const char*)src_ptr + (LAST_XFER ? (GATHER ? index_offset4 : dma4_src_offs) : (GATHER ? index_offset4 : dma4_src_iter_offs)));
        if (DO_SYNC)
            CUDADMA_BASE::wait_for_dma_start();
        *(TYPE1*)((char*)dst_ptr + (LAST_XFER ? (GATHER ? dma1_dst_offs : index_offset1) : (GATHER ? dma1_dst_iter_offs : index_offset1))) = tmp1;
        *(TYPE1*)((char*)dst_ptr + (LAST_XFER ? (GATHER ? dma2_dst_offs : index_offset2) : (GATHER ? dma2_dst_iter_offs : index_offset2))) = tmp2;
        *(TYPE1*)((char*)dst_ptr + (LAST_XFER ? (GATHER ? dma3_dst_offs : index_offset3) : (GATHER ? dma3_dst_iter_offs : index_offset3))) = tmp3;
        *(TYPE2*)((char*)dst_ptr + (LAST_XFER ? (GATHER ? dma4_dst_offs : index_offset4) : (GATHER ? dma4_dst_iter_offs : index_offset4))) = tmp4;
    }
...
#define THREADS_PER_ELMT (LDS_PER_ELMT > (WARP_SIZE / 2) ? WARP_SIZE : LDS_PER_ELMT > (WARP_SIZE / 4) ? (WARP_SIZE / 2) : LDS_PER_ELMT > (WARP_SIZE / 8) ? (WARP_SIZE / 4) : LDS_PER_ELMT > (WARP_SIZE / 16) ? (WARP_SIZE / 8) : LDS_PER_ELMT > (WARP_SIZE / 32) ? (WARP_SIZE / 16) : WARP_SIZE / 32)
...
#define PARTIAL_ELMTS (SPLIT_WARP ? REMAINING_ELMTS / (DMA_THREADS / THREADS_PER_ELMT) + (int(ELMT_ID_SPLIT) < (REMAINING_ELMTS % (DMA_THREADS / THREADS_PER_ELMT)) ? 1 : 0) : REMAINING_ELMTS / (DMA_THREADS / WARP_SIZE) + (int(ELMT_ID_SPLIT) < (REMAINING_ELMTS % (DMA_THREADS / WARP_SIZE)) ? 1 : 0))
...
#define WARPS_PER_ELMT ((MAX_WARPS_PER_ELMT >= (DMA_THREADS / WARP_SIZE)) ? (DMA_THREADS / WARP_SIZE) : (DMA_THREADS / WARP_SIZE) > (MAX_WARPS_PER_ELMT * NUM_ELMTS) ? (DMA_THREADS / WARP_SIZE) / NUM_ELMTS : MAX_WARPS_PER_ELMT)
...
#define CUDADMASTRIDED_DMA_OFF(stride, id) (SPLIT_ELMT ? (id * (SPLIT_WARP ? DMA_THREADS / THREADS_PER_ELMT : DMA_THREADS / WARP_SIZE) * stride + ELMT_ID_SPLIT * stride) : id * ALIGNMENT * WARPS_PER_ELMT * WARP_SIZE + ALIGNMENT * CUDADMA_WARP_TID)
...
#define STRIDED_EXECUTE(DO_SYNC)                                                                                                                                                              \
    ...
    {                                                                                                                                                                                         \
        ...
        {                                                                                                                                                                                     \
            ...
            {                                                                                                                                                                                 \
                int opt_xfer = (thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) ? (thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) : (thread_bytes ? MAX_BYTES_OUTSTANDING_PER_THREAD : 0); \
                if (all_threads_active)                                                                                                                                                       \
                {                                                                                                                                                                             \
                    if (warp_partial)                                                                                                                                                         \
...
#define CUDADMAINDIRECT_DMA_OFF(stride, id) (SPLIT_ELMT ? (id * (SPLIT_WARP ? DMA_THREADS / THREADS_PER_ELMT : DMA_THREADS / WARP_SIZE) * stride + ELMT_ID_SPLIT * stride) : id * ALIGNMENT * WARPS_PER_ELMT * WARP_SIZE + ALIGNMENT * CUDADMA_WARP_TID)
...
#define INDIRECT_EXECUTE(DO_SYNC)                                                                                                                                                                               \
    ...
    {                                                                                                                                                                                                           \
        ...
        {                                                                                                                                                                                                       \
            ...
            {                                                                                                                                                                                                   \
                cudaDMAIndirectBase<GATHER>::template do_xfer_across_indirect<DO_SYNC>(index_ptr, src_row_ptr, dst_row_ptr, this->dma_split_partial_elmts, this->partial_bytes, 0);                             \
            }                                                                                                                                                                                                   \
            else                                                                                                                                                                                                \
            {                                                                                                                                                                                                   \
                if (DO_SYNC)                                                                                                                                                                                    \
                ...
                if (this->dma_split_partial_elmts > 0)                                                                                                                                                          \
                    cudaDMAIndirectBase<GATHER>::template do_xfer_across_indirect<false>(index_ptr, src_row_ptr, dst_row_ptr, this->dma_split_partial_elmts, this->partial_bytes, 0);                           \
            }                                                                                                                                                                                                   \
        }                                                                                                                                                                                                       \
        else                                                                                                                                                                                                    \
        {                                                                                                                                                                                                       \
            ...
            for (int i = 0; i < DMA_ROW_ITERS_SPLIT; i++)                                                                                                                                                       \
            {                                                                                                                                                                                                   \
                cudaDMAIndirectBase<GATHER>::template do_xfer_across_indirect<false>(index_ptr, src_row_ptr, dst_row_ptr, MAX_LDS_OUTSTANDING_PER_THREAD, this->partial_bytes, offset_index);                   \
            ...
            if (this->dma_split_partial_elmts > 0)                                                                                                                                                              \
                cudaDMAIndirectBase<GATHER>::template do_xfer_across_indirect<false>(index_ptr, src_row_ptr, dst_row_ptr, this->dma_split_partial_elmts, this->partial_bytes, offset_index);                    \
        }                                                                                                                                                                                                       \
    }                                                                                                                                                                                                           \
    else if (ELMT_LDS <= MAX_LDS_OUTSTANDING_PER_THREAD)                                                                                                                                                        \
    {                                                                                                                                                                                                           \
    ...
    {                                                                                                                                                                                                           \
        ...
        {                                                                                                                                                                                                       \
            ...
            if (DMA_COL_ITERS_FULL == 0)                                                                                                                                                                        \
            {                                                                                                                                                                                                   \
                int opt_xfer = (this->thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) ? (this->thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) : (this->thread_bytes ? MAX_BYTES_OUTSTANDING_PER_THREAD : 0); \
                if (this->all_threads_active)                                                                                                                                                                   \
                {                                                                                                                                                                                               \
...
#    define COPY_ACROSS_ELMTS1 cudaDMAIndirectBase<GATHER>::template copy_across_elmts_indirect<BULK_TYPE, DMA_COL_ITERS_SPLIT>(index_ptr, src_row_ptr, dst_row_ptr, this->dma_split_partial_elmts, this->partial_bytes, 0)
#    define COPY_ACROSS_ELMTS2 cudaDMAIndirectBase<GATHER>::template copy_across_elmts_indirect<BULK_TYPE, DMA_COL_ITERS_SPLIT>(index_ptr, src_row_ptr, dst_row_ptr, MAX_LDS_OUTSTANDING_PER_THREAD, this->partial_bytes, offset_index)
#    define COPY_ACROSS_ELMTS3 cudaDMAIndirectBase<GATHER>::template copy_across_elmts_indirect<BULK_TYPE, DMA_COL_ITERS_SPLIT>(index_ptr, src_row_ptr, dst_row_ptr, this->dma_split_partial_elmts, this->partial_bytes, offset_index)
#    define COPY_ELMT_FN cudaDMAStridedBase::template copy_elmt<BULK_TYPE, DMA_COL_ITERS_FULL, ALIGNMENT>(src_row_ptr, dst_row_ptr)
...
#    undef COPY_ELMT_FN
        ...
        else if (ELMT_LDS <= MAX_LDS_OUTSTANDING_PER_THREAD)   // A warp needs to issue multiple loads per element
        {
            ...
            if (DMA_ROW_ITERS_SPLIT == 0)
            {
                if (DO_SYNC)
                    CUDADMA_BASE::wait_for_dma_start();
                cudaDMAIndirectBase<GATHER>::template copy_across_elmts_indirect<BULK_TYPE, DMA_COL_ITERS_SPLIT>(index_ptr, src_row_ptr, dst_row_ptr, this->dma_split_partial_elmts, this->partial_bytes, 0);
            }
            else
            {
                int offset_index = 0;
                ...
                for (int i = 0; i < DMA_ROW_ITERS_SPLIT; i++)
                {
                    cudaDMAIndirectBase<GATHER>::template copy_across_elmts_indirect<BULK_TYPE, DMA_COL_ITERS_SPLIT>(index_ptr,
                                                                                                                     src_row_ptr,
                                                                                                                     dst_row_ptr,
                                                                                                                     MAX_LDS_OUTSTANDING_PER_THREAD,
                                                                                                                     this->partial_bytes,
                                                                                                                     offset_index);
                    src_row_ptr += (GATHER ? 0 : this->dma_src_row_iter_inc);
                    dst_row_ptr += (GATHER ? this->dma_dst_row_iter_inc : 0);
...
protected:
    ...
    __device__ __forceinline__ void execute_internal(const int* RESTRICT index_ptr, const void* RESTRICT src_ptr, void* RESTRICT dst_ptr) const
    {
#define COPY_ACROSS_ELMTS1 cudaDMAIndirectBase<GATHER>::template copy_across_elmts_indirect<BULK_TYPE>(index_ptr, src_row_ptr, dst_row_ptr, this->dma_split_partial_elmts, this->partial_bytes, DMA_COL_ITERS_SPLIT, 0)
#define COPY_ACROSS_ELMTS2 cudaDMAIndirectBase<GATHER>::template copy_across_elmts_indirect<BULK_TYPE>(index_ptr, src_row_ptr, dst_row_ptr, MAX_LDS_OUTSTANDING_PER_THREAD, this->partial_bytes, DMA_COL_ITERS_SPLIT, offset_index)
#define COPY_ACROSS_ELMTS3 cudaDMAIndirectBase<GATHER>::template copy_across_elmts_indirect<BULK_TYPE>(index_ptr, src_row_ptr, dst_row_ptr, this->dma_split_partial_elmts, this->partial_bytes, DMA_COL_ITERS_SPLIT, offset_index)
#define COPY_ELMT_FN cudaDMAStridedBase::template copy_elmt<BULK_TYPE, ALIGNMENT>(src_row_ptr, dst_row_ptr, DMA_COL_ITERS_FULL)
...
#define COPY_ACROSS_ELMTS2 cudaDMAIndirectBase<GATHER>::template copy_across_elmts_indirect<BULK_TYPE>(index_ptr, src_row_ptr, dst_row_ptr, MAX_LDS_OUTSTANDING_PER_THREAD, this->partial_bytes, DMA_COL_ITERS_SPLIT, offset_index)
#define COPY_ACROSS_ELMTS3 cudaDMAIndirectBase<GATHER>::template copy_across_elmts_indirect<BULK_TYPE>(index_ptr, src_row_ptr, dst_row_ptr, this->dma_split_partial_elmts, this->partial_bytes, DMA_COL_ITERS_SPLIT, offset_index)
#define COPY_ELMT_FN cudaDMAIndirectBase<GATHER>::template copy_elmt<BULK_TYPE, ALIGNMENT>(src_row_ptr, dst_row_ptr, DMA_COL_ITERS_FULL)
        INDIRECT_EXECUTE(true)
#undef COPY_ACROSS_ELMTS1
#undef COPY_ACROSS_ELMTS2
#undef COPY_ACROSS_ELMTS3
...
#define REMAINING_WARPS ((num_dma_threads / WARP_SIZE) > (2 * RADIUS * MAX_WARPS_PER_ROW) ? (num_dma_threads / WARP_SIZE) - (2 * RADIUS) * MAX_WARPS_PER_ROW : 0)
...
public:
    __device__ cudaDMAHalo(const int dmaID,
        ...
        , dma_src_top_offset(CORNERS ? (-(RADIUS * pitch + RADIUS) * sizeof(ELMT_TYPE)) : (-(RADIUS * pitch) * sizeof(ELMT_TYPE)))
        , dma_src_bot_offset(CORNERS ? ((dimy * pitch - RADIUS) * sizeof(ELMT_TYPE)) : (dimy * pitch) * sizeof(ELMT_TYPE))
        , dma_dst_top_offset(CORNERS ? (-(RADIUS * (dimx + 2 * RADIUS) + RADIUS) * sizeof(ELMT_TYPE)) : (-(RADIUS * (dimx + 2 * RADIUS) * sizeof(ELMT_TYPE))))
        , dma_dst_bot_offset(CORNERS ? ((dimy * (dimx + 2 * RADIUS) - RADIUS) * sizeof(ELMT_TYPE)) : (dimy * (dimx + 2 * RADIUS) * sizeof(ELMT_TYPE)))
        ...
        , row_thread(ROW_ID < (2 * RADIUS))
    {
        if (REMAINING_WARPS > 0)
        {
            ...
            if (!(ROW_ID < (2 * RADIUS)))
            {
                int local_id      = threadIdx.x - (dma_threadIdx_start + num_dma_threads - (REMAINING_WARPS * WARP_SIZE));
                ...
                side_src_offset   = (side_id % 2 ? dimx * sizeof(ELMT_TYPE) + side_load * SIDE_XFER_SIZE
                                               : -(TOTAL_SIDE_LOADS - side_load) * SIDE_XFER_SIZE) +
                                  (side_id / 2) * pitch * sizeof(ELMT_TYPE);
                side_dst_offset = (side_id % 2 ? dimx * sizeof(ELMT_TYPE) + side_load * SIDE_XFER_SIZE
                                               : -(TOTAL_SIDE_LOADS - side_load) * SIDE_XFER_SIZE) +
                                  (side_id / 2) * (2 * RADIUS + dimx) * sizeof(ELMT_TYPE);
...
private:   // Helper methods
    ...
    __device__ __forceinline__ void load_rows(int& row_id, const char* RESTRICT src_row_ptr, char* RESTRICT dst_row_ptr) const
    {
        ...
        case 4:
        {
            for (; row_id < MAX_ROW; row_id += dma_row_iters)
            {
                ...
                // Handle the leftovers
                CUDADMA_BASE::template do_xfer<false, ALIGNMENT>(src_temp, dst_temp, (thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) ? (thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) : (thread_bytes ? MAX_BYTES_OUTSTANDING_PER_THREAD : 0));
    ...
    {
        CUDADMA_BASE::template do_xfer<DO_SYNC, ALIGNMENT>(src_row_ptr, dst_row_ptr, (thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) ? (thread_bytes % MAX_BYTES_OUTSTANDING_PER_THREAD) : (thread_bytes ? MAX_BYTES_OUTSTANDING_PER_THREAD : 0));
    }
